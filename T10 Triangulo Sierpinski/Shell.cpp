...
BOOL Get_xy(int *x, int *y)
{
  ...
  do
  {
	 WaitMessage();
	 HWND__ *_temp;
	 PeekMessage(&msg,_temp,0,0,PM_REMOVE);
...
void Window::Grafica(Matrix<float> &F,int x0,int y0,float Tam)
{
  ...
  for(j=0;j<R-1;j++)
  {
	for(i=0;i<C;i++)
    {
        P[i].x  =(LONG)(x0+0.8*(-j        +2.0*i)*Tam      +50);P[i].y  =(LONG)(y0-F(i,j)+Tam*j);
        P[i+C].x=(LONG)(x0+0.8*(-j-1+2.0*(C-1-i))*Tam+50);P[i+C].y=(LONG)(y0-F(C-1-i,j+1)+Tam*(j+1));
    }
	Polygon(MemDC,P,2*C);
	Polygon(hdc,P,2*C);
  }
...
void Window::Grafica3D(float **F,int x0,int y0,float Tam,int nr,int nc,float fact)
{
  ...
  for(j=0;j<R-1;j++)
  {
	for(i=0;i<C;i++)
	 {
		  P[i].x  =(LONG)(x0+0.8*(-j        +2.0*i)*Tam      +50);P[i].y  =(LONG)(y0-F[j][i]*fact+Tam*j);
		  P[i+C].x=(LONG)(x0+0.8*(-j-1+2.0*(C-1-i))*Tam+50);P[i+C].y=(LONG)(y0-F[j+1][C-1-i]*fact+Tam*(j+1));
	 }
	Polygon(MemDC,P,2*C);
	Polygon(hdc,P,2*C);
  }
...
for (i = 0 ; i < nr ; i++)
	for (j = 0 ; j < nc ; j++)
	  if (0) f[i][j] = 0 ;             // antes estaba if(f[i][j] < 0 )
