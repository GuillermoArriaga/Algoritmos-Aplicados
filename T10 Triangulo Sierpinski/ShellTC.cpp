...
/*****************************************************************************/
unsigned char bptr1[3*1024*1024] ;
...
void Window::Grafica(Matrix<float> &F,int x0,int y0,float Tam)
{
  ...
  for(j=0;j<R-1;j++)
  {
	for(i=0;i<C;i++)
    {
        P[i].x  =x0+0.8*(-j        +2.0*i)*Tam      +50;P[i].y  =y0-F(i,j)+Tam*j;
        P[i+C].x=x0+0.8*(-j-1+2.0*(C-1-i))*Tam+50;P[i+C].y=y0-F(C-1-i,j+1)+Tam*(j+1);
    }
	Polygon(MemDC,P,2*C);
	Polygon(hdc,P,2*C);
  }
...
void Window::Grafica3D(float **F,int x0,int y0,float Tam,int nr,int nc,float fact)
{
  ...
  for(j=0;j<R-1;j++)
  {
	for(i=0;i<C;i++)
	 {
		  P[i].x  =x0+0.8*(-j        +2.0*i)*Tam      +50;P[i].y  =y0-F[j][i]*fact+Tam*j;
		  P[i+C].x=x0+0.8*(-j-1+2.0*(C-1-i))*Tam+50;P[i+C].y=y0-F[j+1][C-1-i]*fact+Tam*(j+1);
	 }
	Polygon(MemDC,P,2*C);
	Polygon(hdc,P,2*C);
  }
...
{
    int i,j,tx=B.Cols(),ty=B.Rows(),lx=zoom*((float)tx),ly=zoom*((float)ty);
	...
	for (j = 0 ; j < nc ; j++)
		...
		for (i = 0 ; i < nrp ; i++)
				for (j = 0 ; j < ncp ; j++)
						{
						if (i < nr && j < nc)
						  bptr1[i*ncp+j] =  (f[i][j] - auxm)*aux + 10  ;
						else bptr1[i*ncp+j] = 255 ;
						}
	 lx = zoom*ncp ; ly = zoom*nrp ;
    HDC hdc = GetDC(hWnd);
    HDC hMemDC = CreateCompatibleDC(hdc);
	 //HBITMAP hbit = CreateBitmapGray(hMemDC, tx, ty, b.Ptr());
...
void B(char *name,void (*ButtFunc) ())
{
    ...
    if(ButtonsCount>=20)
    {
        MessageOk("ERROR! no more than 20 Buttons allowed");
        return;
    }
    /*HWND hWndB=*/
...
void Write256GrayBitmapFile(float **M,int nr,int nc,char *name)
{
	...
	for(i=0;i<x;i++)
	    for(int j=0;j<y;j++)
            p[i+j*x]=M[i][y-1-j];//buffer de pixeles
